...
9. `writeMergedCsv` + `writeHtml` — outputs. `writeHtml` reads `scripts/vendor/echarts.min.js` synchronously and inlines it as a single `<script>` block before `</body>` so reports stay offline-capable. Head emits only local styles — no external `<link>` (no Google Fonts); typography relies on the CSS-var fallback stack (`Segoe UI` / `PingFang SC` etc.), with a vendored-Geist follow-up tracked in [.scratch/offline-fonts/issues/01-vendor-woff2.md](.scratch/offline-fonts/issues/01-vendor-woff2.md). Overview section renders the RT cumulative-yield line, SW Bin Pareto (bar + cum-% line, 80% markLine) + SW bin count histogram + SW bin count box plot (5/4/3 row), the HW Bin Pareto on a second full-width row below, and a Yield-by-Site region with a three-chart 5/4/3 row: per-Site FT vs True grouped bar chart (FT `--mute`, True `--accent`), a Site yield histogram (yield% bucketed), and a Site yield box plot (single Tukey box, `--accent` fill, `--accent-2` median, `--fail` outliers, y-axis padded around the data). A global `FT | Final` toggle at the top-right of the Overview header now swaps **six** reactive charts — the four Bin Pareto charts plus the two Yield-by-Site distribution charts (histogram + box plot) — via `applyScope(isFinal)` using precomputed arrays; no network, no recompute. The per-Site FT-vs-True grouped bar chart is independent of the toggle (always shows both bars side-by-side).
